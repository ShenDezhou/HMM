...
def neglogviterbi(a_seq):
    ...
    start_p = [0.8, 1e-200, 1e-200, 0.2]  # BMES
    ...
    for i_state in range(N):
        if a_seq[0] in dicts.keys():
            ...
            viterbi[i_state, 0] = - numpy.log(start_p[i_state])
    ...
    for i_seq in range(1, T):
        ...
        for i_state in range(N):
            if a_seq[i_seq] in dicts.keys():
                ...
                viterbi[i_state, i_seq] = min(
                    viterbi[:, i_seq - 1] - numpy.log(t[:, i_state]))
                backpointer[i_state, i_seq] = numpy.argmin(
                    viterbi[:, i_seq - 1] - numpy.log(t[:, i_state]), axis=0)
